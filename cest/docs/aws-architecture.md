...
v0.4 で探索モード（最適エリア逆算）と Bedrock（AI分析）を追加。
...
│  demo_         │              ▼
│  response.json │   ┌──────────────────────┐
│  station_      │   │  Lambda Function     │
│  master.json   │   │  (Python 3.11)       │
│                │   │                      │
│                │   │  FastAPI + Mangum    │
│                │   │  networkx            │
│                │   │  pydantic            │
...
| 項目 | v0.3.2 | v0.4 |
|------|--------|------|
| Lambda メモリ | 512MB | **1024MB** |
| Lambda タイムアウト | 30秒 | **60秒** |
| API エンドポイント | /evaluate, /parse-csv | + **/explore, /ask** |
| Bedrock | なし | **Claude 3 Haiku** |
| IAM 権限 | Lambda 基本実行 | + **bedrock:InvokeModel** |
| S3 / CloudFront / CI/CD | 変更なし | 変更なし |
...
| 項目 | 内容 |
|---|---|
| 用途 | `/evaluate` `/parse-csv` `/explore` `/ask` のHTTPエンドポイント |
...
| メモリ | v0.3.2: 512MB / v0.4: 1024MB |
| タイムアウト | v0.3.2: 30秒 / v0.4: 60秒 |
| 選定理由 | サーバー管理不要。トラフィックゼロ時は課金なし |
| 月額コスト | **永久無料**（100万リクエスト/月まで） |

> **EC2ではなくLambdaにした理由**
> EC2 t2.microは12ヶ月後に課金開始（~$10/月）。Lambdaは永久無料枠あり。
> CESTはリクエスト数が少ない用途なのでLambdaが最適。

> **v0.4 でメモリ・タイムアウトを引き上げる理由**
> 探索モードは全候補駅（~800）× 全社員の通勤時間を計算する。
> 512MB/30秒でも動く可能性はあるが、社員数が多い場合のマージンを確保する。
> Step Functions による並列化は、単一 Lambda で処理可能なためオーバーエンジニアリングとして見送る。

### 3-6. Amazon Bedrock（v0.4 で追加）

| 項目 | 内容 |
|---|---|
| 用途 | 分析結果の自然言語Q&A、レポートサマリ生成 |
| モデル | Claude 3 Haiku（低コスト・低レイテンシ） |
| 呼び出し元 | Lambda 関数内から boto3 経由 |
| 選定理由 | AWS マネージドで API キー管理不要。IAM で認可。Lambda から直接呼べる |
| 月額コスト | **~10円**（月100回利用想定） |

> **Bedrock を選んだ理由（外部API ではなく）**
> Lambda → Bedrock は同一 AWS 内の呼び出しのため、APIキーの管理が不要（IAM Role で認可）。
> セキュリティ・運用の観点で、外部のAPI呼び出しより優れている。
> 就活の観点でも「AWSサービスの統合経験」をアピールできる。

> **Bedrock が使えない場合のフォールバック**
> /ask エンドポイントが Bedrock API エラーを返した場合、フロントエンドは
> 「AI分析は現在利用できません」と表示し、ルールベースの分析結果のみを表示する。
...
git push to main
  ...
  └────┬────┘
       ...
       ▼
  ┌──────────────────┐
  │  Deploy Frontend │  aws s3 sync web/ s3://バケット名/
  └────────┬─────────┘  --delete（削除ファイルも反映）
           │
           ▼
  ┌───────────────────────┐
...
| Lambda の権限 | 最小権限。v0.3.2: 基本実行権限のみ。v0.4: + `bedrock:InvokeModel` |
| Bedrock の入力 | ユーザーの自由入力を Bedrock に渡すため、システムプロンプトで応答範囲を制限 |
...
### Phase 3: 探索モード追加（v0.4）

- [ ] エリア賃料相場テーブル作成（`area_rent.py`）
- [ ] 郵便番号→最寄駅マッピングデータ生成（`postal_station_map.json`）
- [ ] 探索ロジック実装（`explore.py` or `combination.py` に追加）
- [ ] `/explore` エンドポイント追加
- [ ] Lambda メモリ 512MB → 1024MB、タイムアウト 30秒 → 60秒
- [ ] フロント探索UI追加
- [ ] `/parse-csv` の郵便番号自動変換対応

### Phase 4: Bedrock 統合（v0.4）

- [ ] Lambda の IAM Role に `bedrock:InvokeModel` 権限追加
- [ ] Bedrock コンソールで Claude 3 Haiku のモデルアクセスを有効化
- [ ] `/ask` エンドポイント実装
- [ ] フロントチャットUI追加
- [ ] PDF レポートへの AI サマリ統合
- [ ] Bedrock エラー時のフォールバック確認
...
- PRごとのプレビュー環境
- 社員セルフ入力フォーム（Cognito + DynamoDB）
- 実経路API連携（NAVITIME等 — 設計上は差し替え可能）
- 結果の永続化（DynamoDB）
