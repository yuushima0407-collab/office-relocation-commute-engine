...
def _make_combo(rent, p95, capacity, combo_id=None, per_office=None):
    """最小限のcomboデータを生成する。"""
    combo = {
        "total_rent_jpy_month": rent,
        "p95_trip_minutes": p95,
...
class TestParetoDominance:
    ...
    def test_equal_two_axes_worse_one(self):
        ...
        A = _make_combo(rent=300, p95=40, capacity=100)
        B = _make_combo(rent=300, p95=40, capacity=90)  # capacity だけ負け
        assert _is_pareto_dominated(B, [A, B]) is True

    def test_capacity_saves_expensive_office(self):
        """収容人数が多いだけで他2軸で負けていてもパレートに残る。
        これがv0.3.1（2軸）では落ちていた問題を解決する。"""
        cheap_small = _make_combo(rent=300, p95=40, capacity=50)
        expensive_big = _make_combo(rent=500, p95=60, capacity=200)
        # rent も p95 も負けてるが capacity で大きく勝つ → 支配されない
    ...
    def test_mark_pareto_frontier_ids(self):
        ...
        combos = [
            _make_combo(rent=300, p95=40, capacity=100),  # パレート
            _make_combo(rent=500, p95=50, capacity=80),   # A に支配される
            _make_combo(rent=400, p95=60, capacity=120),  # capacity で勝ち → パレート
            _make_combo(rent=200, p95=70, capacity=150),  # rent と capacity で勝ち → パレート
        ]
        # num_offices が必要
        for c in combos:
            c["num_offices"] = 1
        ...
        # combos[1] だけが支配される（rent, p95, capacity すべて combos[0] に負ける）
    ...
    def test_all_pareto_when_no_domination(self):
        ...
        combos = [
            _make_combo(rent=100, p95=90, capacity=50),   # 安い・遠い・小さい
            _make_combo(rent=500, p95=30, capacity=80),   # 高い・近い・中
            _make_combo(rent=300, p95=60, capacity=200),  # 中・中・大きい
        ]
        for c in combos:
            c["num_offices"] = 1
        pareto_ids = mark_pareto_frontier(combos)
...
class TestRentTolerance:
    ...
    def test_no_dominator_means_unlimited(self):
        """p95 と capacity で勝てる案がなければ tolerance_pct は None（上限なし）。"""
        combo = _make_combo(rent=300, p95=30, capacity=100, combo_id="test")
        combo["per_office"] = [
            {"office_id": "A", "name": "A社", "rent_jpy_month": 300},
        ]
        # 他の案は p95 で combo に勝てない
        other = _make_combo(rent=200, p95=50, capacity=120)
    ...
    def test_dominator_exists(self):
        ...
        # combo: rent=300, p95=50, cap=100
        # dominator: rent=400, p95=40, cap=120 (p95 ≤ 50 かつ cap ≥ 100 → 支配しうる)
        # combo の rent が 400 を超えたら脱落
        # headroom = 400 - 300 = 100
        combo = _make_combo(rent=300, p95=50, capacity=100, combo_id="test")
        combo["per_office"] = [
            {"office_id": "X", "name": "Xオフィス", "rent_jpy_month": 300},
        ]
        dominator = _make_combo(rent=400, p95=40, capacity=120)
    ...
    def test_multiple_dominators_picks_strictest(self):
        ...
        d1 = _make_combo(rent=400, p95=40, capacity=120)
        d2 = _make_combo(rent=350, p95=45, capacity=110)
    ...
    def test_multi_office_combo(self):
        ...
        combo = _make_combo(rent=500, p95=50, capacity=100, combo_id="test")
        combo["per_office"] = [
            {"office_id": "A", "name": "A社", "rent_jpy_month": 300},
            {"office_id": "B", "name": "B社", "rent_jpy_month": 200},
        ]
        # dominator: rent=600 → headroom = 600 - 500 = 100
        dominator = _make_combo(rent=600, p95=40, capacity=120)
        result = _compute_rent_tolerance(combo, [combo], [dominator])
...
class TestConflictAlerts:
    """部署間の通勤格差が閾値を超えたら警告を出す。"""

    def test_gap_over_threshold(self):
        """p95 の差が 15 分以上なら警告。"""
        breakdown = [
            {"group": "営業部", "p95_trip_minutes": 68},
            {"group": "経理部", "p95_trip_minutes": 45},
        ]
    ...
    def test_gap_under_threshold(self):
        """p95 の差が 15 分未満なら警告なし。"""
        breakdown = [
            {"group": "営業部", "p95_trip_minutes": 50},
            {"group": "経理部", "p95_trip_minutes": 45},
        ]
        alerts = _compute_conflict_alerts(breakdown)
        assert len(alerts) == 0
    ...
    def test_three_departments_multiple_alerts(self):
        ...
        breakdown = [
            {"group": "営業部", "p95_trip_minutes": 70},
            {"group": "経理部", "p95_trip_minutes": 40},
            {"group": "開発部", "p95_trip_minutes": 50},
        ]
        alerts = _compute_conflict_alerts(breakdown)
        # 営業-経理: 30分差 → 警告
        # 営業-開発: 20分差 → 警告
